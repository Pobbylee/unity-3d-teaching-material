...
이런식으로 이차원 배열도 선언할 수 있어요. 삼차원배열은 각자 해보도록 해요.

### 컬렉션 (collection)

Array는 런타임[^1]시에 배열의 지정한 크기를 바꿀 수 없어요. 실행 전 크기를 `int[] arr = new int[10]`이런 문법으로 크기를 선언시에 고정시켜요. 하지만 우리가 만들 게임에서 한번 생각을 해봅시다. 필드에 3초마다 몬스터가 나온다고 하면 그 몬스터들의 갯수를 얼마나 늘어날 지 감이 잡히나요? 보통은 이런때에 실시간으로 늘어나는 배열을 쓰게되요. 이럴때 쓰이는 것이 컬렉션 입니다.
...
> Collection 에는 Generic이 포함되는데, 이 Generic은 아래의 Collection들을 보면 알 수 있겠지만 뒤에 `<T>`라는게 붙어요. 이 `<T>`는 모든 데이터 타입이 들어간다는 뜻이에요.
